...
export function AudioPlayer({ src, title, downloadName, preacher }: AudioPlayerProps) {
  ...
  return (
    <div
      data-audioplayer
      className="bg-slate-100 rounded-lg overflow-hidden px-4 flex items-center gap-3 sm:gap-4 h-[60px]"
    >
      {/* Hidden native audio */}
      ...
      >
        ...
        {/* Thumb */}
        <div
          className="absolute h-3 w-3 rounded-full bg-ibcd-blue shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      {src && (
        <a
          ...
          className="shrink-0 text-slate-400 hover:text-slate-800 transition-colors outline-none focus-visible:text-ibcd-blue"
          title="Descargar MP3"
        >
          <Download size={18} />
        </a>
